...
namespace Simple.Permissions
{
    ...
    public class Deny<TIdentity> : IPermission<TIdentity>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="Deny{TIdentity}" /> class.
        /// </summary>
        public Deny()
        { }
        ...
        public Deny(TIdentity trustee, Right permissionMask)
        {
            Trustee = trustee;
        ...
        public Right CheckAccess(TIdentity trustee, ref Right accumulator, ref Right result)
        {
            ...
            if (!EqualityComparer<TIdentity>.Default.Equals(trustee, Trustee)) return result; // irrelevant

            // is our mask still relevant?
            // get permissions that are applied from me
            // I won't touch what was already accumulated - calculated
            var effectivePermissions = PermissionMask & ~accumulator;
            if (effectivePermissions != 0)
            {
                result &= ~effectivePermissions; // and remove it from result
